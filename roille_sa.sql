...
FOR EACH ROW
BEGIN
    IF NEW.types = 'particulier' THEN
        INSERT INTO particulier (idclient, prenom, nom, adr, tel, email, mdp) VALUES (NEW.idclient, NEW.prenom, NEW.nom, NEW.adr, NEW.tel, NEW.email, NEW.mdp);
    ELSEIF NEW.types = 'entreprise' THEN
        INSERT INTO entreprise (idclient, siret, statut, tel, nom, adr, email, mdp) VALUES (NEW.idclient, NEW.siret, NEW.statut, NEW.tel, NEW.nom, NEW.adr, NEW.email, NEW.mdp);
    END IF;
END //
DELIMITER ;
